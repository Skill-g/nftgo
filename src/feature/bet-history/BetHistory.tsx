    ...
    return (
        <div className="flex flex-col gap-2">
            <ul className="divide-y divide-white/5 rounded-lg overflow-hidden ring-1 ring-white/10 bg-white/5">
                {items.map(bet => {
                    ...
                    if (bet.status === 'cancelled') {
                        badgeLabel = i18n._(msg`Отменено`)
                        badgeKind = 'cancelled'
                    } else if (bet.status === 'pending') {
                        badgeLabel = i18n._(msg`В процессе`)
                        badgeKind = 'pending'
                    } else if (win || bet.status === 'cashed') {
                        badgeLabel = i18n._(msg`Выиграл`)
                        badgeKind = 'win'
                    } else {
                        badgeLabel = i18n._(msg`Проиграл`)
                        badgeKind = 'loss'
                    }
            ...
            {hasNext && (
                ...
                >
                    {loadingMore ? i18n._(msg`Загрузка...`) : i18n._(msg`Загрузить ещё`)}
                </button>
            )}
        </div>
    )
