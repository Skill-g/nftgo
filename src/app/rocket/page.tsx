...
export default function Page() {
    ...
    const [bets, setBets] = useState<Bet[]>([{ amount: 20, placed: false, betId: null }, { amount: 20, placed: false, betId: null }]);
    const [gamePhase, setGamePhase] = useState("waiting");
    const [currentMultiplier, setCurrentMultiplier] = useState(1);
    const [roundId, setRoundId] = useState<number | null>(null);

    const placeBet = useCallback(async (index: number) => {
    ...
    const onCashOut = useCallback(async (index: number) => {
        if (!user?.initData || !roundId) return;
